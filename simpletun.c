...
///////////////////////////////////////////////////////////////
	...
	if(!EVP_EncryptUpdate(ctx, encoutbuf, &outlen, buffer, nread))
       		{
        	/* Error */
...
///////////////////////////////////////////////////////////////
      ...
      /* write length + packet */
      //plength = htons(strlen(encoutbuf));
      //nwrite = cwrite(net_fd, (char *)&plength, sizeof(plength));
      nwrite = cwrite(net_fd, sendencoutbuf, outlen+16+64);
      
      do_debug("TAP2NET %lu: Written %d bytes to the network with %d pl\n", tap2net, nwrite, strlen(encoutbuf));
    }
    ...
    if(FD_ISSET(net_fd, &rd_set)){
      ...
      /* read packet */
      nread = cread(net_fd, buffer, BUFSIZE);
...
////////////	Generation Of hash starts here	///////////////
///////////////////////////////////////////////////////////////
	unsigned char * buf_dec_ptr = buffer+16;
	HMAC(EVP_sha256(), key, strlen(key), buffer+16, nread-16-64, md_value1, &md_len1 );
	unsigned char * dec_ptr = printbuf;
	for(i =0;i < md_len1;i++){
		dec_ptr+=sprintf(dec_ptr,"%02x",md_value1[i]);
	}
	printbuf[64]='\0';
	unsigned char * ptr = buffer+(nread - 64);
	for(i = nread - 64, j =0 ;i < nread;i++, j++){
		printbuf2[j]=buffer[i];
	}
	printbuf2[64]='\0';
	/*
...
///////////////////////////////////////////////////////////////
	...
	realbufpointer = buffer + 16;
	
	EVP_DecryptInit_ex(dctx, EVP_aes_256_cbc(), NULL, key, iv);
	if(!EVP_DecryptUpdate(dctx, decoutbuf, &decoutlen, realbufpointer, nread-16-64))
       		{
        	/* Error */
...
///////////////////////////////////////////////////////////////
      ...
      nwrite = cwrite(tap_fd, decoutbuf, decoutlen);
